...
export async function POST(request: Request) {
  try {
    const { friendEmail, friendName, userName, groupName } = await request.json();
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [friendEmail],
      subject: `You've been added to a new group: ${groupName}`,
      html: `
        <div style="font-family: sans-serif; background-color: #05050A; color: #F8F9FA; padding: 40px; border-radius: 12px; max-width: 600px; margin: 0 auto; border: 1px solid rgba(255,255,255,0.05);">
          <h2 style="color: #7C3AED; margin-bottom: 20px;">New Group Invitation</h2>
          <p>Hey ${friendName},</p>
          <p><strong>${userName}</strong> just created a new group called <strong>${groupName}</strong> and added you to it.</p>
          
          <div style="margin: 30px 0; text-align: center;">
            <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/groups" style="background: #7C3AED; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">View Groups</a>
          </div>
